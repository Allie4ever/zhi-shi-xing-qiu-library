...
export default function Home() {
  ...
  const [uploadForm, setUploadForm] = useState({ route: "due-diligence" as Route, savePdf: true });
  ...
  async function importPdf() {
    ...
    try {
      ...
      const materialDate = `${localNow.getFullYear()}-${String(localNow.getMonth() + 1).padStart(2, "0")}-${String(localNow.getDate()).padStart(2, "0")}`;
      const material: Material = {
        id: `local-${crypto.randomUUID()}`, route: uploadForm.route, manager: "待人工整理", title: selectedFile.name.replace(/\.pdf$/i, "") || "待命名材料",
        materialDate, materialType: uploadForm.route === "due-diligence" ? "尽调报告" : "路演材料",
  ...
  return <main>
    ...
    <div className="workspace">
      ...
      <section className="hero"><div><p className="kicker">RESEARCH ARCHIVE / 研究档案</p><h1>把每一份材料，<em>变成可验证的判断。</em></h1></div><div className="hero-stats"><div><strong>{materials.length}</strong><span>材料总数</span></div><div><strong>{materials.filter((item) => item.sourceType === "private-local").length}</strong><span>本机私有</span></div><div><strong>{materials.filter((item) => item.sourceType === "local").length}</strong><span>临时导入</span></div></div></section>
    ...
    {uploadOpen && <div className="modal-backdrop" onMouseDown={() => !uploading && setUploadOpen(false)}><section className="upload-modal" role="dialog" onMouseDown={(event) => event.stopPropagation()}><button className="modal-close" onClick={() => setUploadOpen(false)}>×</button><p className="section-kicker">BROWSER-ONLY INGEST</p><h2>本地导入一份PDF</h2><p className="modal-intro">PDF仅在当前浏览器处理，默认最大20MB，不会发送到GitHub或任何云端。标题自动使用文件名，管理人与其他信息可稍后整理。</p><div className="type-choice">{tabs.map((tab) => <button key={tab.id} className={uploadForm.route === tab.id ? "active" : ""} onClick={() => setUploadForm((form) => ({ ...form, route: tab.id }))}>{tab.label}</button>)}</div><button className={`dropzone ${selectedFile ? "has-file" : ""}`} onClick={() => fileInputRef.current?.click()}><span className="upload-symbol">{selectedFile ? "✓" : "↑"}</span><strong>{selectedFile?.name || "选择本地PDF"}</strong><small>{selectedFile ? `${(selectedFile.size / 1024 / 1024).toFixed(2)} MB` : "仅支持.pdf，最大20MB"}</small></button><input ref={fileInputRef} hidden type="file" accept="application/pdf,.pdf" onChange={handleFile} /><label className="save-option"><input type="checkbox" checked={!uploadForm.savePdf} onChange={(event) => setUploadForm((form) => ({ ...form, savePdf: !event.target.checked }))} /> 仅保存正文、不保存PDF原件</label><button className="confirm-upload" disabled={!selectedFile || uploading} onClick={importPdf}>{uploading ? "正在本地提取与保存…" : "开始本地导入"}</button></section></div>}
